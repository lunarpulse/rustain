...
//! trait method. The on-disk representation is human-readable markdown (see
//! `adapters/long_term_memory.rs`), NOT serde-JSON, so `serde` is intentionally
//! NOT derived — facts round-trip through the adapter's markdown parser.
