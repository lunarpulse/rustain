...
pub fn render(area: Rect, buf: &mut Buffer, plan_title: &str, n_pending: u32, n_completed: u32) {
    ...
    if area.height > 2 {
        buf.set_string(
            ...
            area.y + 2,
            format!(
                "│ {} task(s) remaining, {} completed (will be kept)",
                n_pending, n_completed
            ),
            style,
        );
    }
