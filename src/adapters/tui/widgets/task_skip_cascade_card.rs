...
pub fn render(
    ...
    if area.height > 1 {
        buf.set_string(area.x, area.y + 1, format!("│ Task {} skipped — {} task(s) depend on it.", pending.source_task, count), style);
    }
    if area.height > 2 {
        buf.set_string(area.x, area.y + 2, "│ [s] Skip them too  [c] Continue anyway  [n] Cancel skip", style);
    }
    if area.height > 3 {
