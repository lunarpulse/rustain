...
mod tests {
    ...
    fn test_single_newline_renders_as_two_lines() {
        ...
        assert!(all_text.contains("line one"), "'line one' missing from: {all_text:?}");
        assert!(all_text.contains("line two"), "'line two' missing from: {all_text:?}");
        ...
        assert!(non_blank[0].contains("line one"), "First content line wrong: {:?}", non_blank[0]);
        assert!(non_blank[1].contains("line two"), "Second content line wrong: {:?}", non_blank[1]);
    }
    ...
    fn test_multiline_user_input_preserves_all_lines() {
        ...
        let non_blank: Vec<String> = lines
            ...
            .filter(|s| !s.trim().is_empty())
            .collect();
        assert_eq!(non_blank.len(), 3, "Expected 3 non-blank lines: {non_blank:?}");
    ...
    fn test_markdown_formatting_preserved_across_newlines() {
        ...
        let non_blank: Vec<String> = lines
            ...
            .filter(|s| !s.trim().is_empty())
            .collect();
        assert_eq!(non_blank.len(), 2, "Expected 2 non-blank lines: {non_blank:?}");
        let all: String = lines
            .iter()
            .flat_map(|l| l.spans.iter().map(|s| s.content.as_ref()))
            .collect();
        assert!(all.contains("bold text"), "Bold text content missing: {all:?}");
    ...
    fn test_inline_code_preserved_with_newlines() {
        ...
        let all: String = lines
            ...
            .flat_map(|l| l.spans.iter().map(|s| s.content.as_ref()))
            .collect();
        assert!(all.contains("code here"), "Code span content missing: {all:?}");
