...
- **WindowingAssembler (Story 11.6, 2026-06-01):** the second concrete `ContextAssemblerPort` implementation, sibling to `StaticPassthroughAssembler` (Story 11.0). Within-session-only token optimization via deterministic turn grouping (R1 tool-chain integrity / R3 time-gap / R4 file-Jaccard / R5 tool-chain break) + relevance-weighted LRU trim (`0.5·recency + 0.5·signature_overlap_with_active_anchor`). **Design discipline:** no PPR machinery, no user-facing tunables — keep complexity off the settings surface (Lunarpulse, 2026-06-01). Realises CMV "structurally lossless trim" via group topology (R1 guarantees tool-call/tool-result pairs share a group, so the group-granular LRU trim cannot orphan them). Promotion to Algorithm A++ (full PPR) is gated by ADR-11-2 deferral triggers: `expand_group` tool shipping, Algorithm B committing, or sustained session size G > 50 in telemetry.

**Clarify before acting.** Before executing any request, first restate it in precise, technical, and professional English to lock down scope. Convert vague or informal input into concrete, unambiguous requirements, then proceed.
