...
- **Task execution (Story 6.2a):** `PlanRuntime` (`src/domain/services/plan_runtime.rs`) consumes `AppEvent::PlanExecutionStarted` from 6-1a and walks `Plan.tasks` sequentially. Each task is dispatched as a synthetic `AgentThenSubmit` turn — the agent's tool calls flow through the existing `ToolScheduler`. Per-task status (`PlanTaskStatus`), elapsed time, and result are stored on `PlanTask`. Failures auto-skip downstream-dependent tasks (FR82); a unified summary `ContentBlockType::PlanSummary` message is appended on completion (FR81). Parallel/sub-task decomposition is deferred to Story 10.6 (rehomed from 6-2b).
- **Task panel (Story 6.3):** `task_panel` widget renders a sidebar list of plan tasks driven by `AppEvent::PlanTaskStatusChanged` (from 6-2a) and reads task state from `conversation.plans[plan_id]`. `Ctrl+X, T` chord toggles visibility; `auto_panels.on_task_plan = "tasks"` triggers auto-open on `PlanExecutionStarted`. Drill-down to a `task_detail` view replaces the chat pane; `Esc` returns. Per-tool granularity stays in the chat-pane `tool_block` widget; the task panel only renders task-level status. Multi-panel sidebar dispatcher branches on `state.sidebar_panel` (Tasks/History/Agents/Adapters).
- **Task control (Story 6.4):** panel keymap `p` (Pause/Resume) / `s` (Skip) / `x` (Cancel plan); drill-down keymap `r` (Retry) / `s` (Skip) / `e` (Edit) on Failed; `p` (Resume) on Paused. Palette commands `!cancel-plan` / `!reorder-task` / `!resume-all-tasks`. New `PlanTaskStatus::Paused` (non-terminal); new `AppEvent::PlanDeviation`; new `PlanDeviationKind` enum. `PlanRuntimeState` extended with `pause_pending_tasks`, `whole_plan_cancel_pending`, `pending_deviation`. `on_turn_complete::Cancelled` branch split three ways. Cards render inline via chat-pane render priority chain: deviation > cancel-confirm > skip-cascade > drill-down > chat.
